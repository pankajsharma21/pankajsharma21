...
rows, clips, css = [], [], []
t = 0.35                                   # running start time, seconds
for i, (prompt, text, cls) in enumerate(LINES):
    y = TOP + i * LH
    full = prompt + text
    width = max(len(full) * CW, 1)
    dur = max(len(full) * 0.045, 0.18) if full else 0.12
    clips.append(
...
<style>
  ...
  .hi  {{ fill:#7fb0ff }}
  .num {{ fill:#ffd15c }}
  .cur {{ fill:#5cff9d; animation: blink 1.05s steps(1) 3.4s infinite }}
